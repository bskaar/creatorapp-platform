  ...
  const loadSites = async () => {
    ...
    try {
      ...
      if (memberships && memberships.length > 0) {
        ...
        const { data, error: memberSitesError } = await supabase
          .from('sites')
          .select('*')
          .in('id', siteIds);

        if (memberSitesError) {
          console.error('Error loading member sites:', memberSitesError);
          throw memberSitesError;
        }
        memberSites = data || [];
      }
      ...
      const allSites: SiteWithPlan[] = await Promise.all(
        allSitesRaw.map(async (site) => {
          if (site.platform_subscription_plan_id) {
            ...
            if (planError) {
              console.error('Error loading subscription plan:', planError);
            }
